...
<head>
    ...
    <title>General ALPC TR Dashboard</title>
...
<body>
    ...
    <div class="top-header">
        ...
        <div class="header-center">
            <div class="monitoring-title">
                ...
                <div class="monitoring-subtitle">ALPC TR<?php echo isset($_GET['lpc']) ? ' - LPC ' . htmlspecialchars($_GET['lpc']) : ' - All LPCs'; ?> - Overview & Analytics</div>
            </div>
        </div>
        ...
        <div class="main-content">
            ...
            <div class="filter-section" style="margin-bottom: 12px; gap: 8px; display: flex; flex-wrap: wrap; align-items: center;">
                ...
                <select id="filter-lpc" class="filter-input" style="padding: 6px; font-size: 12px;">
                    <option value="all" <?php echo !isset($_GET['lpc']) || $_GET['lpc'] == 'all' ? 'selected' : ''; ?>>All LPCs</option>
                    <option value="1" <?php echo isset($_GET['lpc']) && $_GET['lpc'] == '1' ? 'selected' : ''; ?>>LPC 1</option>
                    <option value="2" <?php echo isset($_GET['lpc']) && $_GET['lpc'] == '2' ? 'selected' : ''; ?>>LPC 2</option>
                    <option value="3" <?php echo isset($_GET['lpc']) && $_GET['lpc'] == '3' ? 'selected' : ''; ?>>LPC 3</option>
                    <option value="4" <?php echo isset($_GET['lpc']) && $_GET['lpc'] == '4' ? 'selected' : ''; ?>>LPC 4</option>
                    <option value="5" <?php echo isset($_GET['lpc']) && $_GET['lpc'] == '5' ? 'selected' : ''; ?>>LPC 6</option>
                </select>
                ...
                <button class="filter-btn active" onclick="GeneralALPCTR.loadData()" style="padding: 6px 16px; font-size: 12px; margin-left: 8px;">
                    Apply Filter
                </button>
                ...
                <button class="filter-btn" onclick="backToOverview()" style="padding: 6px 12px; font-size: 12px; background: var(--accent-navy); color: white;">
                    ← Back to Overview
                </button>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 12px;">
                ...
                <!-- Gate Average -->
                <div class="metric-card">
                    <div class="metric-label" style="font-size: 11px;">Gate Sensors Avg</div>
                    <div class="metric-value" id="gate-avg" style="font-size: 24px;">--</div>
                    <div class="metric-unit" style="font-size: 12px;">°C</div>
                    <span class="status-badge status-normal" id="status-gate" style="font-size: 10px; padding: 3px 8px;">--</span>
                </div>

                <!-- Chamber Average -->
                <div class="metric-card">
                    <div class="metric-label" style="font-size: 11px;">Chamber Sensors Avg</div>
                    <div class="metric-value" id="chamber-avg" style="font-size: 24px;">--</div>
                    <div class="metric-unit" style="font-size: 12px;">°C</div>
                    <span class="status-badge status-normal" id="status-chamber" style="font-size: 10px; padding: 3px 8px;">--</span>
                </div>
            </div>
    ...
    <script src="../js/general-alpc-tr.js"></script>
    <script>
        // Reset filters
        function resetFilters() {
            ...
            GeneralALPCTR.loadData();
        }

        // Back to overview dashboard
        function backToOverview() {
            window.location.href = 'alpc-overview.php';
        }
        ...
        function toggleRealTimeMonitoring() {
            ...
            if (isRealTimeEnabled) {
                ...
                // Start simulation
                GeneralALPCTR.startSimulation(3);
